...
namespace S3_Example
{
    class Program
    {
        ...
        static readonly string bucketName = ConfigurationManager.AppSettings["AWS_PUBLIC_BUCKET"] ?? "";
        static readonly string accessKey = ConfigurationManager.AppSettings["AWS_ACCESS_KEY"] ?? "";
        static readonly string secretKey = ConfigurationManager.AppSettings["AWS_ACCESS_SECRET_KEY"] ?? "";
        ...
        static void Main(string[] args)
        {
            ...
            using (var s3 = new AmazonS3Client(accessKey, secretKey, RegionEndpoint.USEast1))
            {
                WriteObject(s3);
        ...
        static void WriteObject(IAmazonS3 s3)
        {
            ...
            var keyruntime = "sub-directory/FILE-NAME-" + $"{DateTime.Now:yyyyMMddmmss}.jpeg";
            ...
            var req = new PutObjectRequest
            {
                BucketName = bucketName,
                Key = keyruntime,
                FilePath = "C:\\Users\\Hecto\\Downloads\\descargar.jpeg"
            };
            ...
            if (res.IsCompletedSuccessfully)
            {
                var acl = s3.PutACLAsync(new PutACLRequest
                {
                    BucketName = bucketName,
                    Key = keyruntime,
            ...
            Task.WaitAny(acl);
                Console.WriteLine("Created object '{0}' in bucket '{1}'", keyruntime, bucketName);
            }
        }
